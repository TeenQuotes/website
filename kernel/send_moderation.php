<?php			
$query = mysql_query("SELECT * FROM approve_quotes WHERE send = '0' AND id_user = '".$author_id."'");
...
while ($data = mysql_fetch_array($query))
{
	...
	{
		if (preg_match('/'.$domain_fr.'/', $_SERVER['SERVER_NAME']))
			$unapproved_quote_txt .= '<div style="background:#f5f5f5;border:1px solid #e5e5e5;padding:10px;margin:30px 10px">'.$texte_quote.'<br/><br/><a href="http://'.$domain.'" target="_blank">#'.$id_quote.'</a><span style="float:right">par <a href="http://'.$domain.'/user-'.$author_id.'" target="_blank">'.$name_auteur.'</a> le '.$date_quote.'</span></div>';
		elseif (preg_match('/'.$domain_en.'/', $_SERVER['SERVER_NAME']))
			$unapproved_quote_txt .= '<div style="background:#f5f5f5;border:1px solid #e5e5e5;padding:10px;margin:30px 10px">'.$texte_quote.'<br/><br/><a href="http://'.$domain.'" target="_blank">#'.$id_quote.'</a><span style="float:right">by <a href="http://'.$domain.'/user-'.$author_id.'" target="_blank">'.$name_auteur.'</a> on '.$date_quote.'</span></div>';

		$nb_quote_unapproved++;
	}	
} // END WHILE

$update_send = mysql_query("UPDATE approve_quotes SET send = '1' WHERE id_user = '".$author_id."'");
