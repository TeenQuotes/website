...
echo '
<div class="post">
<h1><img src="http://'.$domain.'/images/icones/profil.png" class="icone" />'.$edit_comment.'</h1>
';
	...
	if ((mysql_num_rows($query_comment) == 1 AND $id_auteur == $id) OR $_SESSION['security_level'] >= 2)
	{
		if (empty($action))
		{
			...
			<div class="grey_post">
				...
				<a href="quote-'.$id_quote.'">#'.$id_quote.'</a>'; afficher_favori($id_quote, $is_favorite, $logged, $_SESSION['id']);date_et_auteur($auteur_id, $auteur, $date_quote, $on, $by, $view_his_profile);
			echo '</div>';
