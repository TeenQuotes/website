...
else
{
	...
	if ($num_rows_quote >= 1 OR $num_rows_members >= 1) 
	{
		...
		<div class="post">
			<div class="grey_post">
				...
				{
					echo '<h3 class="no_margin_left"><a href="#quotes"><img src="http://'.$domaine.'/images/icones/profil.png" class="icone">'.$quotes.'</a><span class="right"><a href="#members"><img src="http://'.$domaine.'/images/icones/staff.png" class="icone">'.$members.'</span></a></h3>';
				}
			echo '</div>';
		echo '</div>';
				...
				while ($result = mysql_fetch_array($resultatsQuotes))
				{
					...
					<div class="grey_post">
						...
						<div class="footer_quote">
							<a href="quote-<?php echo $result['id']; ?>">#<?php echo $result['id']; ?> - <?php afficher_nb_comments ($nombre_commentaires, $comments, $comment, $no_comments); ?></a><?php afficher_favori($id_quote, $is_favorite, $logged, $add_favorite, $unfavorite); date_et_auteur ($auteur_id,$auteur,$date_quote,$on,$by,$view_his_profile); ?>
						</div>
					</div>
					<?php 
				}
	...
	echo '
	<div class="post">
		<h2><img src="http://'.$domaine.'/images/icones/search_result.png" class="icone" />'.$no_result.'</h2>
		'.$no_result_fun.'
	</div>';
	}
}
