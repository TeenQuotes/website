	...
	if (mysql_num_rows($query_comment) == 1 AND $id_auteur == $id)
		...
		if (empty($action))
			...
			<div class="grey_post">
				...
				<a href="quote-'.$id_quote.'">#'.$id_quote.'</a>'; afficher_favori($id_quote,$is_favorite,$logged,$add_favorite,$unfavorite,$_SESSION['account']);date_et_auteur($auteur_id,$auteur,$date_quote,$on,$by,$view_his_profile);
			echo '</div>
			</div>';
