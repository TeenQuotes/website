...
$display_page_top = display_page_top($donnees['nb_messages'], $nbPostsPerPage, 'p', $previous_page, $next_page, null, true);
...
else
{
	$reponse = mysql_query("SELECT q.texte_english texte_english, q.id id, q.auteur_id auteur_id, q.date DATE, a.username auteur, COUNT( c.id ) nb_comments
							...
							WHERE q.approved =  '1'
							GROUP BY q.id
							ORDER BY  `q`.`id` LIMIT $premierMessageAafficher, $nbPostsPerPage");
}
